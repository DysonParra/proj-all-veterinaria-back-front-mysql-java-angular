/*
 * @fileoverview    {ChatDetalleRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ChatDetalleRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ChatDetalleRepository extends JpaRepository<ChatDetalle, Long> {
     ...
     */
    public List<ChatDetalle> findByIntIdChatDetalle(Long id);
    ...
    @Query("SELECT m FROM ChatDetalle m WHERE m.intIdChatDetalle LIKE CONCAT('%', :strSearch, '%')")
    public List<ChatDetalle> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM ChatDetalle m WHERE m.intIdChatDetalle LIKE CONCAT('%', :strSearch, '%')")
    public Page<ChatDetalle> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
